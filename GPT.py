...
def main(args):
    ...
    loss_log_file = open("loss_log.log", "w")
    ...
    for epoch in range(args.epoch):
        ...
        for input_ids, output_labels, mask_for_loss in tqdm(data_loader, desc=f"Training epoch {epoch}"):
            ...
            loss_log_file.write(f"Batch Loss: {loss_lm.item()}\n")
            loss_log_file.flush() 
        ...
        if loss_v < max(best_val_losses, key=lambda x: x[0])[0]:
            ...
            # 保存当前模型权重
            current_model_path = f"{args.model_path}_v0_Epoch_{epoch}.pth"
